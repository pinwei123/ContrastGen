...
class Trainer(object):
    def __init__(
        ...
        dataset,
        val_dataset,
        early_stop_patience=10,  # 連續幾次沒進步就停止
        val_every=500,  # 每幾步驗證一次
        ema_decay = 0.995,
        image_size = 128,
        depth_size = 128,
        train_batch_size = 2,
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                sampleImage=sampleImage.reshape([ self.depth_size, self.image_size, self.image_size])
            ...
            if self.step % self.val_every == 0 and self.val_ds is not None:
                self.validate()
                if self.early_stop:
    ...
    def validate(self):
        ...
        val_loader = data.DataLoader(self.val_ds, batch_size=1, shuffle=False)
        losses = []
        ...
        for batch in val_loader:
            ...
            loss = self.model(target_tensor, condition_tensors=input_tensor)
            losses.append(loss.item())

        avg_val_loss = np.mean(losses)
        self.writer.add_scalar("val_loss", avg_val_loss, self.step)

        print(f"Validation loss at step {self.step}: {avg_val_loss:.6f}")
        ...
        if avg_val_loss < self.best_val_loss:
            ...
            print(f"→ No improvement for {self.epochs_no_improve}/{self.early_stop_patience} evaluations.")
